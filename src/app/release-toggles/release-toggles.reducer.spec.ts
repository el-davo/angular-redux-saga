...
describe('Release Toggles Reducer', () => {
  ...
  it('should start fetching release toggles', () => {
    ...
    expect(releaseTogglesReducer(releaseToggleState, action)).toEqual({...releaseToggleState, isFetchingReleaseToggles: true});
  });
  ...
  it('should finish fetching release toggles successfully', () => {
    ...
    expect(releaseTogglesReducer(releaseToggleState, action)).toEqual({...releaseToggleState, releaseToggles: [{}], isFetchingReleaseToggles: false});
  });

  it('should indicate fetching release toggles failed', () => {
    const action = {type: actionTypes.FETCH_RELEASE_TOGGLES_FAILED};
    expect(releaseTogglesReducer(releaseToggleState, action)).toEqual({...releaseToggleState, isFetchingReleaseToggles: false});
  });

  it('should show edit toggle modal', () => {
    const action = {type: actionTypes.SHOW_EDIT_TOGGLE_MODAL, releaseToggle: {}};
    expect(releaseTogglesReducer(releaseToggleState, action)).toEqual({...releaseToggleState, editReleaseToggle: {}, showEditToggleModal: true});
  });
  ...
  it('should hide edit toggle modal', () => {
    ...
    expect(releaseTogglesReducer(state, action)).toEqual({...releaseToggleState, showEditToggleModal: false, editReleaseToggle: {}});
  });
  ...
  it('should indicate toggle edit succeeded', () => {
    ...
    expect(releaseTogglesReducer(state, action)).toEqual({...releaseToggleState, releaseToggles: [{ id: '1'}, {id: '2', name: 'abc'}]});
  });
